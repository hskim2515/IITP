...
>
> **재검증 결과(2026-07-30, scenario2_1) — 2번 결함 미재현**: 위 절연 로직이 실제로 2번
> 결함 상황에서 개입하는지 확인하려고, 강남역 인근 소규모 네트워크(375노드/449링크)에서
> 실제 터미널 쌍으로 3~8링크(정류장 1개 포함) 노선 6가지를 순차 실행했으나 **전부 크래시
> 없이 정상 완주했다**(`test-fixtures/nextsim-crash-repro/case2-terminal-to-terminal-negative-result/`
> 참고). 즉 2번 결함은 순수 링크 개수가 아니라 2026-07-27 조사가 이루어진 scenario3_1의
> 특정 네트워크 토폴로지에 국한된 문제일 가능성이 높다 — 원 문서의 "위상에 따라 재현된다"는
> 서술과 일치. 반대로 이번 재검증 중 **다른 크래시 하나를 새로 확정**했다: 버스 노선의
> `station.seq`가 빈 문자열이면(정류장이 하나도 안 걸리면) 터미널 조합과 무관하게 항상
> `Cannot build OD map for line N. RoadLinks or RoadStations is empty.`로 SIGSEGV — 위
> "정류장이 하나도 안 걸린 노선" 항목과 같은 부류의 결함이나, 이번엔 수동 편집/직접 API
> 호출 경로로 재현됨(`OsmFacilityConverter`의 빈 station.seq 노선 제외 로직을 안 거침).
> 재현 데이터는 `test-fixtures/nextsim-crash-repro/case1-empty-station-seq/` 참고.
