...
import { NETWORK_TILING, RAIL_STATION_TILING, NETWORK_LOD_TIER_ORDER, getNetworkLodTierByResolution, getNetworkLodTierByViewWidth } from "@utils/lodConstants";
import axiosInstance from "@api/axiosInstance";

/** 철도정류장 타일 페이로드 (RailPublicTransitResponse.railStations 부분집합) */
export interface RailStationTilePayload {
    railStations: any[];
}

interface TileEntry {
    payload: RailStationTilePayload;
    lastUsed: number;
}

export interface RailStationTileManagerCallbacks {
    onTileLoaded: (tileKey: string, payload: RailStationTilePayload) => void;
    onTileEvicted: (tileKey: string, payload: RailStationTilePayload) => void;
}

/**
 * 철도정류장 BBox 타일 매니저 (읽기 전용). 버스정류장/신호 타일 매니저와 동일 격자(TILE_DEG)·
 * LRU evict 패턴.
 *
 * 서버: `GET /public-transit/station/rail/{versionId}/tiles?bbox=west,south,east,north`
 */
export class RailStationTileManager {
    ...
    constructor(
        ...
        private callbacks: RailStationTileManagerCallbacks,
    ) {}
    ...
    update(extent3857: Extent, resolution: number): void {
        ...
        if (NETWORK_LOD_TIER_ORDER[tier] < NETWORK_LOD_TIER_ORDER[RAIL_STATION_TILING.MIN_TIER]) {
            this.clear();
            return;
        }
    ...
    private fetchTile(key: string, stamp: number): void {
        ...
        const p = axiosInstance
            .get(`/public-transit/station/rail/${this.versionId}/tiles`, { params: { bbox } })
            .then((res) => {
                const payload: RailStationTilePayload = { railStations: res.data?.railStations ?? [] };
            ...
            .catch((err) => {
                if (err?.response?.status === 404) {
                    ...
                    console.warn(`[RailStationTileManager] 타일 fetch 실패 ${key}`, err);
                }
            })
