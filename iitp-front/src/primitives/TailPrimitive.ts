...
export default class TailPrimitive {
    ...
    createResources() {
        // 초기 위치 설정
        this.trailPosition = Array(this.MAX_TRAIL_LENGTH).fill(this.positions[0].clone());
    ...
    updateTrail(newPosition) {
        // 1. trail에 새로운 위치 추가
        this.trailPosition.push(newPosition);

        if (this.trailPosition.length > this.MAX_TRAIL_LENGTH) {
            this.trailPosition.shift();
        }
        ...
        const offsetFactor = (3 / this.trailPosition.length)

        const offsets = this.trailPosition.flatMap((_, i) => [- i * offsetFactor, i * offsetFactor]);

        // 4. 기존 vertexBuffer에 복사 (GPU 데이터 갱신)
        this.vertexArray._attributes[0].vertexBuffer.copyFromArrayView(
            new Float32Array(flattenedTrailPositions)
        );

        this.vertexArray._attributes[1].vertexBuffer.copyFromArrayView(
            new Float32Array(fadeFactors)
        );

        this.vertexArray._attributes[2].vertexBuffer.copyFromArrayView(
            new Float32Array(offsets)
        );

        this.drawCommand.uniformMap.u_time = () => performance.now() / 1000.0
