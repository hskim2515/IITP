...
export default class BusRouteWeekendDataSourceLayer {
                ...
                if (positions.length >= 2) {
                    this.dataSource.entities.add(new Entity({
                        polyline: {
                            positions,
                            width: 2,
                            material: new ColorMaterialProperty(Color.fromCssColorString('#ffaa00')),
                            clampToGround: true,
                        },
                        properties: { id: line.id, interval: line.interval, featureType: "busRouteWeekend" },
                    }));
                }
            }
        } finally {
