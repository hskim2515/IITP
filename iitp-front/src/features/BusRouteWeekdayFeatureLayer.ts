...
export default class BusRouteWeekdayFeatureLayer extends VectorLayer {
    ...
    public async load(): Promise<void> {
        ...
        for (const line of data.lines) {
            ...
            if (coords.length >= 2) {
                ...
                f.setProperties({ id: line.id, interval: line.interval, featureType: "busRouteWeekday" });
                features.push(f);
            }
        }
        this.source.clear();
        this.source.addFeatures(features);
        console.log(`[BusRouteWeekdayFeatureLayer] 로드 완료: ${features.length}개 노선`);
    }
