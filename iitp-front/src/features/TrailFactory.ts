...
export default class VehicleFactory {
    ...
    initializeTrailLine(features) {
        ...
        this.tripSource.addFeatures(trailFeatures)
    }

    /** Vehicle Source 변경 -> Trip Source 반영 */
    updateAnimation() {
        if (!this.running) return;

        // this.vehicleSource.getFeatures().forEach((feature) => {
        this.vehicleSource.getFeatures().forEach((feature) => {
    ...
    updateTrailLine(vehiclePoint: Feature) {
        ...
        if(!trailId) return;
        let trailFeature = this.tripSource.getFeatureById(trailId) as Feature<LineString>;
        if (!trailFeature) {
            trailFeature = new Feature(new LineString([coord,coord]));
            trailFeature.setId(trailId);
            this.tripSource.addFeature(trailFeature);
            return;
        }
